...
LABEL_COLOR = "#190033"
TOTAL_EXPRESSION_FONT_STYLE=("Arial",20)
CURRENT_EXPRESSION_FONT_STYLE=("Arial", 45, "bold")
...
class Calculator:
    ...
    def create_display_labels(self):
        total_label = tk.Label(self.display_frame, text=self.total_expression, anchor=tk.E, bg=DISPLAY_BG, fg=LABEL_COLOR,padx=24, font=TOTAL_EXPRESSION_FONT_STYLE)
        total_label.pack(expand = True, fill = "both")
        
        label = tk.Label(self.display_frame, text=self.current_expression, anchor=tk.E, bg=DISPLAY_BG, fg=LABEL_COLOR,padx=24 ,font=CURRENT_EXPRESSION_FONT_STYLE)
    ...
    def create_digit_buttons(self):
        for digit, grid_value in self.digits.items():
            button = tk.Button(self.buttons_frame, text = str(digit), bg = DIGITS_BG, fg = LABEL_COLOR, font = DIGITS_FONT_STYLE, borderwidth = 0, command = lambda x =digit :self.add_to_expression(x))
